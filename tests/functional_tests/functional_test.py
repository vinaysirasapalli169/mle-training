...
def test_ingest():
    ...
    os.system(
        f"python src/house_price/ingest_data.py --raw {raw} --processed {processed}"
    )
...
@pytest.mark.skipif(
    os.path.isfile("tests/temp/articrafts/LinearRegression.pkl"),
...
def test_score(cleanup):
    ...
    os.system(
        f"python src/house_price/score.py -d {dataset} -m {models} --mae --rmse --log-path {log_file}"
    )
